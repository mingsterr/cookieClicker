...
public class CookieClicker extends JFrame {
    ...
    Building wheatFarm;
    boolean wheatFarmUnlocked;

    Building sugarCane;
    boolean sugarCaneUnlocked;
    ...
    public CookieClicker() {
        ...
        factory = new Building("Factory", 0, 10, 500);
        factoryUnlocked = false;

        wheatFarm = new Building("Wheat Farm", 0, 15, 2500);
        wheatFarmUnlocked = false;

        sugarCane = new Building("Sugar Cane Plantation", 0, 20, 22500);
        sugarCaneUnlocked = false;
        ...
        getMoreBuildings.scheduleAtFixedRate(new TimerTask() {
            public void run() {
                ...
                if (wheatFarmUnlocked == false && factory.getLevel() >= 5) {
                    wheatFarm.unlock();
                    wheatFarmUnlocked = true;
                }
                if (sugarCaneUnlocked == false && wheatFarm.getLevel() >= 10) {
                    sugarCane.unlock();
                    sugarCaneUnlocked = true;
                }
            }
        }, 0, 2000);
        ...
        produceWithBuildings.scheduleAtFixedRate(new TimerTask() {
            public void run() {
                cookies += bakery.getProductionRate()
                        + robot.getProductionRate()
                        + factory.getProductionRate()
                        + wheatFarm.getProductionRate()
                        +sugarCane.getProductionRate();
            }
        }, 0, 1000);
